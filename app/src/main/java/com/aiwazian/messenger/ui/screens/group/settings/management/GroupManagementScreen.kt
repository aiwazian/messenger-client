...
fun GroupManagementScreen(
    ...
    LaunchedEffect(Unit) {
        viewModel.uiEffect.collect { effect ->
            when (effect) {
                GroupManagementEffect.NavigateToMain -> {
                    navBackStack.clear()
                    navBackStack.add(AppRoute.Main)
                }
                
                is GroupManagementEffect.ShowSnackbar -> {
    ...
    Scaffold(
        topBar = {
            PageTopBar(
                title = { Text(stringResource(R.string.group_management)) },
        ...
        Column(modifier = Modifier.padding(innerPadding)) {
            SectionContainer {
                SectionItem(
                    ...
                    onClick = {
                        navBackStack.add(AppRoute.GroupTransferOwnership(groupId = groupId))
                    }
                )
        ...
        if (uiState.showDeleteDialog) {
            AppDialog(
                ...
                content = {
                    Text(stringResource(R.string.delete_group_confirm_message))
                }
            )
        }
    }
