...
class SettingsDateOfBirthViewModel @Inject constructor(
    ...
    private val _effect = MutableSharedFlow<SettingsDateOfBirthEffect>()
    ...
    fun applyExceptionSelection(selection: PrivacyExceptionSelection) {
        if (selection.field != PrivacyField.DATE_OF_BIRTH) {
            return
        }
    ...
    fun onSaveClick() {
        viewModelScope.launch {
            try {
                privacyRepository.updateDateOfBirthPrivacy(
                    _currentLevel.value,
                    _currentExceptions.value
                ).onSuccess {
                    _effect.emit(SettingsDateOfBirthEffect.Back)
                }.onFailure {
    ...
    private fun loadExceptions() {
        viewModelScope.launch {
            privacyRepository.getPrivacySettings().onSuccess { settings ->
                ...
                val exceptions = settings.exceptionsFor(PrivacyField.DATE_OF_BIRTH)
